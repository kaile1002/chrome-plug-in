...
(function ($) {
    ...
    var updateTabStatus = function (active, callable) {
        ...
        //设置显示的icon图标
        chrome.browserAction.setIcon(active ? config.icon.active : config.icon.inactive, callable);
    };
    ...
    setInterval(function () {
        chrome.tabs.query({url: '*://rd.zhaopin.com/resumepreview/resume/viewone/*', status: 'complete'}, function (tabs) {
    ...
    chrome.runtime.onMessage.addListener(
        function (request, sender, sendResponse) {
            ...
            if (request.jd === 'utilEnable') {
                sendResponse({farewell: config.active && config.getHRTabIds().indexOf(sender.tab.id) > -1});
                ...
                if (config.activeHRTabIds[String(request.siteTypeCd)]) {
                    ...
                    if (request.url.indexOf("58.com") > -1) {
                        chrome.tabs.create({url: "https://vip.58.com/logout"}, function (tab) {
                            setTimeout(function () {
                ...
                //查询是否有加载的查看简历页面
                chrome.tabs.query({url: request.url, status: request.status}, function (tabs) {
                ...
                let a={
                    ...
                    "sign":(md5(Date.parse(new Date())+md5('api-headhunter-opt-key').toUpperCase())).toUpperCase(),
                    "data_str":request.data.data_str
                }
                console.log('通过接口发送数据123》》》》',a);
                $.ajax({
                    url: request.url,
                    data: a,
                    cache: false,
                ...
                xhr.onload = function (e) {
                    if (this.readyState == 4) {
                        ...
                        sendResponse({code: -1, msg: "简历下载失败:" + JSON.stringify(this.response)});
                    }
                };
    ...
    chrome.tabs.onUpdated.addListener(function (tabid, changeInfo, tab) {
        ...
        } else if (tab.url.indexOf('cvsehire.51job.com') > -1 && tab.status === 'complete') {
            chrome.tabs.sendMessage(tab.id, {action: 'processForm'}, function (res) {
                chrome.tabs.remove(tab.id);
